...
MODEL_ALIASES: Dict[str, Dict[str, str]] = {
    "qwen3-vl-4b": {
        ...
        "url": "https://huggingface.co/bartowski/Qwen_Qwen3-VL-4B-Instruct-GGUF/resolve/main/Qwen3-VL-4B-Instruct-Q4_K_M.gguf?download=true",
    },
    "qwen3-vl-4b-mmproj": {
        "filename": "mmproj-Qwen3-VL-4B-Instruct-F16.gguf",
        "url": "https://huggingface.co/bartowski/Qwen_Qwen3-VL-4B-Instruct-GGUF/resolve/main/mmproj-Qwen3-VL-4B-Instruct-F16.gguf?download=true",
    },
    "gpt-oss-20b": {
        "filename": "gpt-oss-20b-MXFP4.gguf",
        "url": "https://huggingface.co/bartowski/openai_gpt-oss-20b-GGUF/resolve/main/gpt-oss-20b-MXFP4.gguf?download=true",
    },
}
...
class SimpleGGUFLoader:
    ...
    def _print_progress(self, current: int, total: int, desc: str = "") -> None:
        """WyĂ„â€šĂ˘â‚¬ĹľÄ‚â€žĂ˘â‚¬Â¦Ă„â€šĂ‹ÂÄ‚ËĂ˘â‚¬ĹˇĂ‚Â¬Ă„Ä…ÄąĹźwietla pasek postÄ‚â€žĂ˘â‚¬ĹˇÄ‚ËĂ˘â€šÂ¬ÄąÄľĂ„â€šĂ‹ÂÄ‚ËĂ˘â€šÂ¬ÄąÄľÄ‚â€ąĂ‚Âpu w konsoli"""
        ...
        try:
            while not stop_event.is_set():
                with self._progress_lock:
                    ...
                    bar_len = 30
                    filled = int((self._progress_pct / 100) * bar_len)
                    ...
                    line = f"   {frame} loading {self._progress_pct:3d}% [{bar}] {self._progress_current}/{self._progress_total}  {elapsed}s{tail}"
    ...
    def _load_llama_with_timeout(
        ...
        base_units = max(0, (attempt - 1) * 100)
        started = time.time()
        while worker.is_alive():
            elapsed = time.time() - started
            if elapsed >= max(timeout_s, 1):
                break
            ratio = min(0.99, elapsed / max(timeout_s, 1))
            current_units = min(total_units, base_units + int(ratio * 100))
            self._print_progress(current_units, total_units, desc)
    ...
    def download_model(
        ...
        bytes_read = 0
        total_bytes = 0
        print(f"Pobieranie modelu: {filename}")
        print(f"URL: {url}")
        ...
        try:
            ...
            raise RuntimeError(f"Blad HTTP {exc.code}: {exc.reason} (URL: {url})") from exc
        except URLError as exc:
            raise RuntimeError(f"Blad polaczenia: {exc} (URL: {url})") from exc
        finally:
        ...
        if bytes_read == 0:
            if os.path.exists(partial):
                os.remove(partial)
            raise RuntimeError("Pobrano 0 bajtow.")
        ...
        if header != b"GGUF":
            ...
            raise RuntimeError("Pobrany plik nie jest poprawnym modelem GGUF.")
    ...
    def load(self, model_name: str, show_try_errors: bool = False) -> bool:
        ...
        try:
            ...
            llama_version = getattr(llama_cpp, "__version__", "0.0.0")
            arch = _read_gguf_architecture(model_path)
            if arch and _is_arch_known_unsupported(arch, llama_version):
                print(
                    f"WARNING: Architecture '{arch}' may not be supported by llama-cpp-python {llama_version}."
                )
                print("   Hint: run 'update' to refresh llama-cpp-python.")
                if arch == "qwen3vl":
                    print("   Hint: qwen3vl support depends on llama.cpp build/version.")
                if _is_env_flag_enabled("RUN_AI_STRICT_ARCH_CHECK", default=False):
                    print("   Strict arch check enabled (RUN_AI_STRICT_ARCH_CHECK=1).")
                    return False
                print("   Continuing load attempt anyway.")
            ...
            for attempt, params in enumerate(param_combinations, 1):
                ...
                try:
                    ...
                    self.model = self._load_llama_with_timeout(
                        llama_cpp.Llama,
                        llama_kwargs,
                        timeout_s=timeout_s,
                        attempt=attempt,
                    ...
                    model_size = f"{file_size_mb:.1f} MB"
                    ctx_size = llama_kwargs['n_ctx']
                    gpu_layers = llama_kwargs['n_gpu_layers']
                    threads = llama_kwargs['n_threads'] or "auto"

                    self._progress_newline()
                    print(f"SUCCESS: Model {Colors.OKGREEN}{model_name}{Colors.ENDC} loaded successfully!")
                    print(f"   - Size: {model_size}")
                    print(f"   - Context: {ctx_size} tokens")
                    print(f"   - GPU layers: {gpu_layers if gpu_layers != -1 else 'all'}")
                    print(f"   - Threads: {threads}")
                    print(f"   - Profile: {desc}")
                    if hasattr(self.model, 'model_desc'):
                        print(f"   - Model description: {self.model.model_desc}")
                    return True
...
def show_help():
    ...
    print("\nBASIC:")
    print("  load                - Choose model from list (enter number)")
    print("  load <name>         - Load model by file name")
    print("  models              - Show available GGUF models")
    print("  catalog             - Show downloadable model aliases")
    print("  download <alias|url> [file.gguf] - Download model to ./models/")
    print("  pull <alias|url> [file.gguf]     - Alias for download")
    print("  unload              - Unload model from RAM")
    print("  chat <text>         - Send message to loaded model")
    print("  ai                  - Enter chat mode (AI> prompt, Esc to exit)")
    print("  (loaded model)      - Plain text is treated as chat message")
    ...
    if not HAS_AI_ENGINE:
        print("  install             - Install AI engine")
    else:
        print("  update              - Update llama-cpp-python")
    print("  version             - Show llama-cpp-python version")
    print("  exit                - Exit program (with confirmation)")
...
def _is_escape_input(text: str) -> bool:
    ...
    return value in {"esc", "/esc", ":esc", "exit", "quit"} or ("\x1b" in (text or ""))
...
def run_terminal_chat_session() -> None:
    ...
    print("\nChat mode started. Type Esc (or 'esc') and press Enter to exit.")
    while True:
        try:
            user_text = input("AI> ")
        except KeyboardInterrupt:
...
def main():
    ...
    try:
        while True:
            try:
                prompt_label = "AI> " if (loader and loader.current_model) else "> "
                raw_command = input(f"\n{prompt_label}").strip()
                if not raw_command:
                    continue
                ...
                if command_name in ('exit', 'quit', 'wyjdz'):
                    ...
                    try:
                        ...
                        print(f"SUCCESS: Downloaded model: {result['name']}")
